...
class Stopwatch extends Component {
    ...
    /* 60 secs in 1 min, so... I should multiply it by 60%, because 1/60 */

    render() {
        const { onCount } = this.state; /*render my count up, starting at original state*/
        let seconds = ("0" + (Math.floor(onCount / 1000) % 60).slice - 2); /* keeping count with no decimals, to make sure slicing the last 2 digits */
